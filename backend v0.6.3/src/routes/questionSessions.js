...
questionSessionRouter.post("/", async (req, res) => {
  ...
  db.prepare(
    `INSERT INTO question_sessions
     ...
     VALUES (?, ?, ?, ?, ?, ?, ?)`
  ).run(sessionId, req.user.sub, initial_description, kind || null, "active", now, now);
...
questionSessionRouter.post("/:sessionId/answer", (req, res) => {
  ...
  const session = db
    ...
    .get(sessionId);
  if (!session || (session.owner_id && session.owner_id !== req.user.sub)) {
